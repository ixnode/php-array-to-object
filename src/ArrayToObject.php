...
namespace Ixnode\PhpArrayToObject;
...
class ArrayToObject
{
    ...
    protected function translate(string $key): void
    {
        ...
        if (($key === self::KEY_WIDTH && $this->has(self::KEY_HEIGHT) && $this->has(self::KEY_ASPECT_RATIO))) {
    ...
    public function __call(string $name, array $arguments): string|int|float|bool|null
    {
        ...
        $namingConventions = new NamingConventions($key);

        /* Try to find camelcase key. */
        $camelCaseKey = $namingConventions->getCamelCase();
        if ($this->has($camelCaseKey)) {
            return $this->get($camelCaseKey);
        }

        /* Try to find minus separated key. */
        $separatedKey = $namingConventions->getSeparated();
